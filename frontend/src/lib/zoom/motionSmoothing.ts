// Primitiva de spring (muelle amortiguado) para suavizar cámara y cursor.
//
// Portado conceptualmente desde getopenscreen/openscreen (MIT) —
// src/lib/zoomMath/motionSmoothing.ts. OpenScreen delega en la librería `motion`;
// aquí se REIMPLEMENTÓ como integrador semi-implícito por substeps fijos para
// evitar añadir la dependencia. Mantiene la misma API pública.
...
/** Substep fijo de integración (240 Hz) → estable incluso con deltas grandes. */
...
/**
 * Avanza un eje del spring hacia `target` durante `deltaMs` (tiempo de contenido).
 * Devuelve el nuevo valor y muta `state`. Puede sobrepasar levemente el target en
 * un frame puntual; el clamps de overshoot vive en zoomSpring.ts.
 */
export function stepSpringValue(
  ...
  // Integración semi-implícita (Euler) en substeps fijos de 240 Hz.
  ...
  while (remaining > 0) {
    ...
    const acceleration =
      (-config.stiffness * (state.value - target) - config.damping * state.velocity) / config.mass
...
/** Config del spring del cursor según el factor de suavizado (0..2). */
...
/** Config del spring de la cámara de zoom (valores de Screen Studio). */
