// Geometría pura del foco de zoom: clamp, zona segura y suavizado de bordes.
// Portado desde getopenscreen/openscreen (MIT) — src/lib/zoomMath/focusUtils.ts

import { clamp, clamp01 } from './mathUtils'
import { ZOOM_DEPTH_SCALES, clampFocus, type ViewportRatio, type ZoomDepth, type ZoomFocus } from './types'
...
function easeIntoBoundary(normalized: number): number {
  const t = clamp01(normalized)
  return -t * t * t + 2 * t * t
}

function softClampToRange(value: number, min: number, max: number, softness: number): number {
...
/**
 * Márgenes del foco para una escala dada: impide que al hacer zoom aparezcan
 * "bordes vacíos" fuera de la imagen.
 */
...
/** Igual que clampFocusToScale pero con desaceleración suave en los bordes. */
...
/** Convierte un foco normalizado al stage en coordenadas normalizadas del video. */
